...
const creditors: Creditor[] = [
  ...
  },
  {
    ...
    email: "operacao@horizonte.local",
    telefone: "(21) 3222-2002",
    observacao: null,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
  },
  {
    ...
    email: "suporte@prisma.local",
    telefone: "(31) 3555-3003",
    observacao: null,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
...
const wallets: Wallet[] = [
  {
    ...
    codigo: "PREMIUM",
    descricao: "Carteira de alta prioridade com foco em ticket elevado.",
    credor_id: creditors[0].id,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
  },
  {
    ...
    credor: creditors[0].nome,
    codigo: "PJ",
    descricao: "Base empresarial com ciclo de cobranca mais consultivo.",
    credor_id: creditors[0].id,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
  },
  {
    ...
    codigo: "SUL",
    descricao: "Operacao de varejo dedicada a clientes da regiao sul.",
    credor_id: creditors[1].id,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
  },
  {
    ...
    codigo: "NORTE",
    descricao: "Carteira de varejo com maior volume de contratos abertos.",
    credor_id: creditors[2].id,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
  },
  {
    ...
    credor: creditors[2].nome,
    codigo: "RECUP",
    descricao: "Clientes em recuperacao com tratamento operacional especial.",
    credor_id: creditors[2].id,
    ativo: true,
    criado_em: timestampString(subMonths(today, 6)),
    atualizado_em: timestampString(today),
...
const goals: Goal[] = operators.map((operator, index) => ({
  ...
  credor_id:
    wallets.find((wallet) => wallet.id === operator.primary_wallet_id)?.credor_id ?? null,
...
for (let offset = 0; offset < 45; offset += 1) {
  ...
  operators.forEach((operator, operatorIndex) => {
    ...
    if (shouldCreatePayment) {
      ...
      const officeFees = Number((amount * 0.12).toFixed(2));
      const revenueType = dayIndex % 4 === 0 ? "COLCHAO" : "NOVO";
      payments.push({
        ...
        valor_escritorio: officeFees,
        tipo_receita: revenueType,
        tipo_receita_origem: "automatico",
